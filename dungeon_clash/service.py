"""Application service: wires the run engine to storage and the clock.

This is the seam the CLI and TUI call. It owns the *impure* bits — where the
save file lives, what "now" is, how wall-clock maps to ticks — so the run engine
underneath stays a pure, reproducible function. Time is always passed in, never
read here, which keeps the service testable without sleeping.
"""
...
from dungeon_clash.passive.registry import resolve_strategy
from dungeon_clash.run.events import EncounterStarted, RunEnded
from dungeon_clash.run.session import LogEntry, RunSession, advance, new_run_session
...
def _to_row(entry: LogEntry) -> LogRow:
    ...
    if isinstance(ev, AttackResolved):
        ...
        actor = ev.winner
    elif isinstance(ev, EncounterStarted):
        actor = ev.name
...
def start_session(store: Store, *, seed: int, strategy_name: str, now: float) -> RunSession:
    """Begin a fresh run, replacing any existing one."""
    resolve_strategy(strategy_name)  # validate up front (raises on unknown)
    store.reset()
    session = new_run_session(seed, strategy_name)
    store.save_session(
        seed=seed,
...
def catch_up(store: Store, *, now: float, secs_per_tick: float | None = None) -> RunSession | None:
    """Advance the stored run to reflect elapsed real time, then persist.

    Returns the up-to-date run, or ``None`` if none has been started. Only whole
    ticks are consumed; the sub-tick remainder is carried forward, so no time is
    ever lost (GDD §4.3).
    """
    ...
    session, entries = advance(session, target, strategy=strategy, pool=enemy_pool())
...
def persist_turn(store: Store, session: RunSession, entries: list[LogEntry], *, now: float) -> None:
